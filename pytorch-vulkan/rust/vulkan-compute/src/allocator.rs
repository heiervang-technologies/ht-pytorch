...
static BUFFER_REGISTRY: OnceLock<Mutex<BTreeMap<usize, BufferRecord>>> = OnceLock::new();

fn buffer_registry() -> &'static Mutex<BTreeMap<usize, BufferRecord>> {
    BUFFER_REGISTRY.get_or_init(|| Mutex::new(BTreeMap::new()))
}
    ...
    fn get(&mut self, size: usize, current_generation: u64) -> Option<BufferAlloc> {
        ...
        if let Some(queue) = self.buckets.get_mut(&bucket) {
            ...
            let pos = queue.iter().position(|pb| {
                pb.immediately_reusable || pb.freed_at_generation < current_generation
            });
            if let Some(idx) = pos {
...
pub fn alloc_buffer(dev: &VulkanDevice, size: usize) -> Result<BufferAlloc, VkcError> {
    ...
    let current_gen = dev.flush_generation();
    {
        ...
        if let Some(alloc) = pool.get(size, current_gen) {
            ...
            log::debug!(
                "Pool hit: reusing {}B buffer for {}B request (gen safe)",
                alloc.size,
                size
            );
            return Ok(alloc);
