...
static GLOBAL_DEVICE: OnceLock<RwLock<Option<Arc<VulkanDevice>>>> = OnceLock::new();

fn global_device() -> &'static RwLock<Option<Arc<VulkanDevice>>> {
    GLOBAL_DEVICE.get_or_init(|| RwLock::new(None))
}
...
static AUTO_FLUSH_THRESHOLD: OnceLock<u32> = OnceLock::new();

pub fn auto_flush_threshold() -> u32 {
    *AUTO_FLUSH_THRESHOLD.get_or_init(|| {
...
impl Drop for DeviceInitGuard {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            {
                device.free_command_buffers(self.command_pool.get(), &[self.command_buffer.get()]);
            }
...
impl VulkanDevice {
    ...
    fn new() -> Result<Self, VkcError> {
        ...
        if has_push_descriptor_extension {
            ...
            properties.p_next = &mut push_descriptor_properties as *mut _ as *mut _;
        }
        unsafe {
        ...
        let supports_push_descriptors = has_push_descriptor_extension
            && push_descriptor_properties.max_push_descriptors >= storage_buffer_binding_limit;
        ...
        let mut coop_mat_nv_features = vk::PhysicalDeviceCooperativeMatrixFeaturesNV::default();
        ...
        let has_f16 = (api_has_f16 || has_f16_extension) && f16_features.shader_float16 == vk::TRUE;
        let has_16bit_storage = (api_has_16bit_storage || has_16bit_storage_extension)
        ...
        let has_coop_mat_nv = has_coop_mat_nv_extension
            ...
            && Self::supports_cooperative_matrix_tile(&entry, instance, physical_device);
        ...
        let mut enabled_storage_16bit_features = vk::PhysicalDevice16BitStorageFeatures::default()
            .storage_buffer16_bit_access(has_16bit_storage);
        ...
        if has_coop_mat_nv {
            ...
            next_chain = &mut enabled_coop_mat_nv_features as *mut _ as *mut std::ffi::c_void;
        }

        if has_atomic_float {
            extension_names.push(c"VK_EXT_shader_atomic_float".as_ptr());
            enabled_atomic_float_features.p_next = next_chain;
            next_chain = &mut enabled_atomic_float_features as *mut _ as *mut std::ffi::c_void;
        }
        ...
        if has_16bit_storage {
            ...
            next_chain = &mut enabled_storage_16bit_features as *mut _ as *mut std::ffi::c_void;
        }
        ...
        let pool_size = vk::DescriptorPoolSize::default()
            ...
            .descriptor_count(max_descriptor_sets.saturating_mul(MAX_STORAGE_BUFFER_BINDINGS));
    ...
    fn pick_physical_device(instance: &ash::Instance) -> Result<vk::PhysicalDevice, VkcError> {
        ...
        for &dev in &devices {
            ...
            let supports_vulkan_1_2 = vk::api_version_major(props.api_version) > 1
                || (vk::api_version_major(props.api_version) == 1
                    && vk::api_version_minor(props.api_version) >= 2);
    ...
    fn supports_cooperative_matrix_tile(
        ...
        let extension = ash::nv::cooperative_matrix::Instance::new(entry, instance);
        let query = extension
            .fp()
            .get_physical_device_cooperative_matrix_properties_nv;
        let mut property_count = 0;
        if unsafe { query(physical_device, &mut property_count, std::ptr::null_mut()) }
            != vk::Result::SUCCESS
            || property_count == 0
        {
            return false;
        ...
        let mut properties =
            vec![vk::CooperativeMatrixPropertiesNV::default(); property_count as usize];
        if unsafe {
            query(
                physical_device,
                &mut property_count,
        ...
        properties[..property_count as usize]
            .iter()
            .any(|property| {
                property.m_size == 16
                    && property.n_size == 16
    ...
    pub fn push_descriptor(&self) -> Option<&ash::khr::push_descriptor::Device> {
    ...
    where
        F: FnOnce(vk::CommandBuffer, vk::DescriptorPool) -> Result<(), VkcError>,
    {
        ...
        if !state.is_recording {
            ...
            unsafe {
                self.device
                    .begin_command_buffer(state.command_buffer, &begin)
                    .map_err(VkcError::Vulkan)?;
            }
    ...
    pub fn flush(&self) -> Result<(), VkcError> {
        ...
        let result = unsafe {
            self.device
                ...
                .and_then(|()| self.device.reset_fences(&[fence]).map_err(VkcError::Vulkan))
                .and_then(|()| {
                    self.device
...
impl Drop for VulkanDevice {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            self.device
                .free_command_buffers(self.command_pool, &[state.command_buffer]);
