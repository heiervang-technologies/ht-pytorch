...
static PIPELINE_HANDLES: LazyLock<Mutex<HashSet<usize>>> =
    LazyLock::new(|| Mutex::new(HashSet::new()));
static BUFFER_HANDLES: LazyLock<Mutex<HashSet<usize>>> =
    LazyLock::new(|| Mutex::new(HashSet::new()));
...
#[no_mangle]
pub extern "C" fn vkc_device_extensions(
    buf: *mut c_char,
    buf_len: usize,
) -> i32 {
...
pub extern "C" fn vkc_fill_buffer(
    ...
    if size == 0 || size % 4 != 0 || binding.offset % 4 != 0 {
        log::error!(
            "vkc_fill_buffer requires a non-zero, 4-byte-aligned range"
        );
        return -1;
    }
...
pub extern "C" fn vkc_copy_buffer(
    ...
    {
        log::error!(
            "vkc_copy_buffer requires a non-zero, 4-byte-aligned range"
        );
        return -1;
    }
...
pub extern "C" fn vkc_load_shader(
    ...
    match pipeline::load_compute_pipeline(&dev, spirv, num_bindings) {
        Ok(pipeline) => {
            let handle =
                Box::into_raw(Box::new(pipeline)) as *mut std::ffi::c_void;
...
pub extern "C" fn vkc_destroy_shader(pipeline_handle: *mut std::ffi::c_void) -> i32 {
    ...
    let pipeline =
        unsafe { Box::from_raw(pipeline_handle as *mut pipeline::ComputePipeline) };
...
pub extern "C" fn vkc_dispatch(
    ...
    let pipeline_handles = PIPELINE_HANDLES.lock().unwrap();
    if !pipeline_handles.contains(&(pipeline_handle as usize)) {
        return -1;
    }
    let dev = match device::VulkanDevice::global() {
        Some(d) => d,
