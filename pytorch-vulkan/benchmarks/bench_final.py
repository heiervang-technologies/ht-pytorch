...
def run_suite():
    ...
    for dev_name, dev_str in devices:
        ...
        results[dev_name]["Add (1M)"] = bench("Add", lambda: torch.add(a, b), dev_str.split(":")[0])
        ...
        results[dev_name]["MM (256x256)"] = bench("MM 256", lambda: torch.mm(m256_1, m256_2), dev_str.split(":")[0])

        m512_1 = torch.randn(512, 512, device=dev_str, dtype=dtype)
        m512_2 = torch.randn(512, 512, device=dev_str, dtype=dtype)
        results[dev_name]["MM (512x512)"] = bench("MM 512", lambda: torch.mm(m512_1, m512_2), dev_str.split(":")[0])

        m1024_1 = torch.randn(1024, 1024, device=dev_str, dtype=dtype)
        m1024_2 = torch.randn(1024, 1024, device=dev_str, dtype=dtype)
        results[dev_name]["MM (1024x1024)"] = bench("MM 1024", lambda: torch.mm(m1024_1, m1024_2), dev_str.split(":")[0])

        # 3. Softmax (32x256)
        s1 = torch.randn(32, 256, device=dev_str, dtype=dtype)
        results[dev_name]["Softmax (32x256)"] = bench("Softmax", lambda: F.softmax(s1, dim=-1), dev_str.split(":")[0])
        ...
        if LlamaBlock is not None:
            ...
            results[dev_name]["LLaMA Block (f16)"] = bench("LLaMA", lambda: block_dev(x_f16, cos_f16, sin_f16), dev_str.split(":")[0], warmup=2, iters=10)

        # 5. Adam Training Step
        model = nn.Sequential(nn.Linear(128, 256), nn.ReLU(), nn.Linear(256, 128)).to(dev_str)
        ...
        results[dev_name]["Adam Train Step"] = bench("Adam", train_step, dev_str.split(":")[0], warmup=2, iters=10)
