...
def _determine_and_set_global_inference_mode(
    ...
    """Determine global inference mode via all-reduce; if any rank needs DYNAMIC, all use DYNAMIC."""
    needs_dynamic = any(
...
class PipelineScheduleSingle(_PipelineSchedule):
    ...
    def _initialize_stage(self, args, kwargs, target=None):
        ...
        needs_fwd = not self._stage_forward_initialized
        needs_bwd = self._has_backward and not self._stage_backward_initialized

        if not needs_fwd and not needs_bwd:
            return

        if needs_fwd and not p2p_done:
            all_ops: list[dist.P2POp] = list(self._stage._get_init_p2p_neighbors_ops())
            _wait_batch_p2p(_batch_p2p(all_ops))

        if needs_fwd and isinstance(self._stage, PipelineStage):
            _determine_and_set_global_inference_mode([self._stage], self._has_backward)

        # Fork RNG so metadata inference doesn't perturb training RNG.
        stage_device = torch.device(self._stage.device)
        with torch.random.fork_rng(
            devices=[stage_device] if stage_device.type != "cpu" else [],
        ):
            if needs_fwd:
        ...
        # Clean up FSDP side effects once after all inference is done
        if isinstance(self._stage, PipelineStage):
            self._stage._post_metadata_inference_cleanup()
...
class PipelineScheduleMulti(_PipelineSchedule):
    ...
    def _initialize_stages(self, args: tuple[Any, ...], kwargs, target=None):
        ...
        needs_fwd = not self._stages_forward_initialized
        needs_bwd = self._has_backward and not self._stages_backward_initialized
        ...
        if needs_fwd and all(
            isinstance(stage, PipelineStage) for stage in self._stages
        ):
