...
void main() {
  ...
  if (uBlock.dim_info.x == 0) {
    for (int batch = 0; batch < uBlock.dim_info.y; batch++) {
      ...
      out_texel = max(out_texel, texelFetch(uInput, ivec3(pos.x, pos.y, src_z), 0));
    }
  ...
  // Height, Width
    ...
    for (int hw = 0; hw < uBlock.dim_info.y; hw++) {
      vec4 v = (uBlock.dim_info.x == 2)
          ? texelFetch(uInput, ivec3(pos.x, hw, pos.z), 0)
          : texelFetch(uInput, ivec3(hw, pos.y, pos.z), 0);
      out_texel = max(out_texel, v);
    }
    imageStore(uOutput, pos, out_texel);
  }
}
